function [uKnot, vKnot, noCtrPts,noElems,noElemsU,noElemsV, ...
          noDofs, elRangeU, elConnU,elRangeV, elConnV ,element,...
          index,noGPs,controlPts, weights, p, q,noPtsX , noPtsY] = Evaluation_Nurbs_Par(Surf)
...
weights    = reshape(Surf.Weights, 1, []);
controlPts = reshape(Surf.CtrlPts3D, 3, [])';

uKnot = Surf.KntVect{1};
vKnot = Surf.KntVect{2};

noPtsX = Surf.NCtrlPts(1);
noPtsY = Surf.NCtrlPts(2);
